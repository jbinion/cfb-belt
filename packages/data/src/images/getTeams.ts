...
const main = async () => {
	...
	const teamsWithEspnData = allAppearingTeams.map((team) => {
		const espnTeam = espnTeamData.find((espnTeam) => espnTeam.name === normalizeTeamNames(team));
		if (espnTeam) return { ...espnTeam, name: team };
		return { name: team };
	});
	console.log(teamsWithEspnData);

	const urls = teamsWithEspnData.filter((x) => x.hasOwnProperty('espnLogo')).map((x) => x.espnLogo);

	const noImages = teamsWithEspnData.filter((x) => !x.hasOwnProperty('espnLogo'));
